...
def build_session_config(config: Config) -> dict:
    ...
    instructions += (
        ...
        "seconds, so say a brief present-tense line ('on it', 'switching that now') right "
        "as you call it rather than going quiet for the whole wait. ALWAYS use browser_task for web navigation, "
        "search, forms and links; it is the installed browser-use/jev-ultrafast Agent with typesafe-ai/jev. "
        "It can take up to about 90 seconds -- likewise acknowledge it briefly right as you "
        "call it, not just before or after. "
